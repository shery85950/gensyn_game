...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, THEME_COLORS, COLORS } from '../../types';
import { audio } from '../System/Audio';

// Geometry Constants
const OBSTACLE_HEIGHT = 2.2;
const OBSTACLE_GEOMETRY = new THREE.BoxGeometry(1.6, OBSTACLE_HEIGHT, 1.6);
const OBSTACLE_GLOW_GEO = new THREE.BoxGeometry(1.7, OBSTACLE_HEIGHT + 0.1, 1.7);
const OBSTACLE_DETAIL_GEO = new THREE.PlaneGeometry(1.4, 0.1); 

// Compute Token (Gem)
const GEM_GEOMETRY = new THREE.OctahedronGeometry(0.35, 0);

// Worker Node (Alien)
const ALIEN_BODY_GEO = new THREE.SphereGeometry(0.5, 16, 16);
const ALIEN_RING_GEO = new THREE.TorusGeometry(0.7, 0.05, 8, 32);
const ALIEN_ANTENNA_GEO = new THREE.CylinderGeometry(0.02, 0.02, 0.5);

// Data Packet (Missile)
const MISSILE_CORE_GEO = new THREE.BoxGeometry(0.2, 0.2, 2.0);
const MISSILE_GLOW_GEO = new THREE.BoxGeometry(0.4, 0.4, 2.2);
...
const SHADOW_MISSILE_GEO = new THREE.PlaneGeometry(0.3, 2);
        ...
        if (obj.type === ObjectType.ALIEN && obj.active && !obj.hasFired) {
             if (obj.position[2] > -90) {
                 ...
                 newSpawns.push({
                     ...
                     active: true,
                     color: COLORS.danger // Bright Red/Orange for missiles
                 });
                 hasChanges = true;
                 
                 window.dispatchEvent(new CustomEvent('particle-burst', { 
                    detail: { position: obj.position, color: COLORS.accent } 
                 }));
             }
        }
                ...
                if (dx < 0.9) { 
                     ...
                     if (isDamageSource) {
                         ...
                         if (isHit) { 
                             ...
                             if (obj.type === ObjectType.MISSILE) {
                                window.dispatchEvent(new CustomEvent('particle-burst', { 
                                    detail: { position: obj.position, color: COLORS.danger } 
                                }));
                             }
                         }
                     } else {
                ...
                if (spawnAlien) {
                    ...
                    for (let k = 0; k < alienCount; k++) {
                        ...
                        keptObjects.push({
                            ...
                            active: true,
                            color: COLORS.danger, // Terracotta for enemies
                            hasFired: false
                        });
                    }
                } else {
    ...
    useFrame((state, delta) => {
        ...
        if (visualRef.current) {
            ...
            if (data.type === ObjectType.SHOP_PORTAL) {
                 ...
                 visualRef.current.position.z += delta * 50; 
                ...
                visualRef.current.rotation.z += delta * 1;
            ...
            <group ref={visualRef} position={[0, data.position[1], 0]}>
                ...
                {data.type === ObjectType.SHOP_PORTAL && (
                    <group>
                         ...
                         <mesh position={[0, 3, 0]} geometry={SHOP_OUTLINE_GEO} scale={[laneCount * LANE_WIDTH + 2.2, 1, 1]}>
                             <meshBasicMaterial color={COLORS.teal} wireframe transparent opacity={0.5} />
                         </mesh>
                         ...
                         <mesh position={[0, 0.1, 0]} rotation={[-Math.PI/2, 0, 0]} geometry={SHOP_FLOOR_GEO} scale={[laneCount * LANE_WIDTH, 1, 1]}>
                             <meshBasicMaterial color={COLORS.teal} transparent opacity={0.3} />
                         </mesh>
                    </group>
                )}
                ...
                {data.type === ObjectType.OBSTACLE && (
                    <group>
                        ...
                        <mesh geometry={OBSTACLE_GLOW_GEO}>
                             <meshBasicMaterial 
                                 color={data.color} 
                                 wireframe 
                                 transparent 
                        ...
                        {/* Blinking Lights */}
                         <mesh position={[0, 0.5, 0.81]} geometry={OBSTACLE_DETAIL_GEO}>
                             <meshBasicMaterial color={data.color} />
                         </mesh>
                         <mesh position={[0, -0.2, 0.81]} geometry={OBSTACLE_DETAIL_GEO}>
                             <meshBasicMaterial color="#221105" />
                         </mesh>
                         <mesh position={[0, -0.6, 0.81]} geometry={OBSTACLE_DETAIL_GEO}>
                             <meshBasicMaterial color={data.color} />
                         </mesh>
                    </group>
                )}

                {/* --- ALIEN (Worker Node Drone) --- */}
                {data.type === ObjectType.ALIEN && (
                    <group>
                        <mesh castShadow geometry={ALIEN_BODY_GEO}>
                            <meshStandardMaterial color={COLORS.background} metalness={0.8} roughness={0.2} />
                        </mesh>
                        <mesh rotation={[Math.PI/2, 0, 0]} geometry={ALIEN_RING_GEO}>
                            <meshBasicMaterial color={data.color} transparent opacity={0.8} />
                        </mesh>
                        <mesh position={[0, 0.5, 0]} geometry={ALIEN_ANTENNA_GEO}>
                             <meshBasicMaterial color={COLORS.structure} />
                        </mesh>
                ...
                {data.type === ObjectType.MISSILE && (
                    <group>
                        <mesh geometry={MISSILE_CORE_GEO}>
                         ...
                         <mesh geometry={MISSILE_GLOW_GEO}>
